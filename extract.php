...
for ($i = 0; $i < count($city_codes); $i++) {
    for ($j = $i+1; $j < count($city_codes); $j++) {
        if ($i == $j) continue;
        ...
        $output = shell_exec('python -u "C:\xampp\htdocs\SL Project\CS699_Project\extract.py" ' . $src . ' ' . $des . ' 2024-10-24 2>&1');        
        if (!$output) {
        ...
        foreach ($lines as $line) {
            ...
            if ($line) {
                ...
                if (count($data) >= 10) {
                    ...
                    $values[] = "('$source_city', '$des_city', '$flight_name', '$flight_duration', '$departure_time', '$time_takeoff', '$time_landing', '$price','$direct_flight_status', '$Flight_date','$websiteLink')";
                }
            }
        }
        ...
        if (!empty($values)) {
            $sql = "INSERT IGNORE INTO scrabbed_data (source_city, destination_city, flight_name, flight_duration, departure_time, time_takeoff, time_landing, price, direct_flight_status, Flight_Date, websiteLink) 
